...
class TestGetOrCreateSession:
    async def test_creates_session_when_none_exists(self, fresh_db):
        session_id = await db_module.get_or_create_session("AAPL")
        assert session_id  # non-empty string

    async def test_returns_same_id_on_second_call(self, fresh_db):
        """One session per ticker — second call must return the same ID."""
        first = await db_module.get_or_create_session("TSLA")
        second = await db_module.get_or_create_session("TSLA")
        assert first == second

    async def test_different_tickers_get_different_sessions(self, fresh_db):
        aapl = await db_module.get_or_create_session("AAPL")
        msft = await db_module.get_or_create_session("MSFT")
        assert aapl != msft

    async def test_normalises_ticker_to_uppercase(self, fresh_db):
        """'aapl' and 'AAPL' must resolve to the same session."""
        lower = await db_module.get_or_create_session("aapl")
        upper = await db_module.get_or_create_session("AAPL")
        assert lower == upper

    async def test_session_stored_in_db(self, fresh_db):
        session_id = await db_module.get_or_create_session("NVDA")
...
class TestGetSessionByTicker:
    async def test_returns_none_when_no_session(self, fresh_db):
        result = await db_module.get_session_by_ticker("XYZ")
        assert result is None

    async def test_returns_session_after_creation(self, fresh_db):
        session_id = await db_module.get_or_create_session("GOOG")
        result = await db_module.get_session_by_ticker("GOOG")
        assert result is not None
        assert result["id"] == session_id
        assert result["ticker"] == "GOOG"

    async def test_case_insensitive_lookup(self, fresh_db):
        await db_module.get_or_create_session("AMZN")
        result = await db_module.get_session_by_ticker("amzn")
        assert result is not None

    async def test_returns_correct_ticker_field(self, fresh_db):
        await db_module.get_or_create_session("META")
        result = await db_module.get_session_by_ticker("META")
...
class TestGetSession:
    ...
    async def test_returns_all_fields(self, fresh_db):
        session_id = await db_module.get_or_create_session("AAPL")
        session = await db_module.get_session(session_id)
        assert set(session.keys()) >= {"id", "ticker", "summary", "created_at"}
...
class TestDeleteSession:
    ...
    async def test_after_delete_new_session_created_for_same_ticker(self, fresh_db):
        ...
        first_id = await db_module.get_or_create_session("AAPL")
        await db_module.delete_session(first_id)
        second_id = await db_module.get_or_create_session("AAPL")
